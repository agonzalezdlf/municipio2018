...
<c:forEach items="${ordenes}" var="item" varStatus="status"> 
  <tr id='f<c:out value="${cont}"/>'>
    ...
    <sec:authorize ifAllGranted="ROLE_Sam_PRIVILEGIOS_APERTURAR_ORDEN_DE_PAGO">
    	<c:if test='${item.STATUS==0&&item.STATUS!=6}'><input alt="<c:out value='${item.NUM_OP}'/>" type="checkbox" id="chkordenes" name="chkordenes" value="<c:out value='${item.CVE_OP}'/>"/></c:if>
    </sec:authorize>
    </td>
    <td align="center">
 ...
 <sec:authorize ifAllGranted="ROLE_Sam_PRIVILEGIOS_APERTURAR_ORDEN_DE_PAGO"> 
 <c:if test="${fn:contains(status,'0')&&item.STATUS!=4}"> 
  <tr>
