...
public class GatewayRequisicion  extends BaseGateway {
	...
	public Long guardar(Long id_proyecto, String num_req, Long cve_contrato, Long cve_vale, String cve_unidad, Date fecha, int tipo, String notas, int mes, int status, int ejercicio , int cve_pers, int anualizada, String proyecto, String partida, int id_grupo) throws Exception{
		Long cve_req = 0L;
		...
		if (c) 
	    	throw new Exception("La requisicion ya existe, esta duplicando el numero de la requisicion" + num_req); 
...
/*------------------------------------------Metodo privado que apertura la requisicion-----------------------------------------------------------------*/
	...
	public Map getFechaIngreso(Long cve_req){
		return this.getJdbcTemplate().queryForMap("SELECT SAM_REQUISIC.NUM_REQ, CONVERT(varchar(10),SAM_REQUISIC.FECHA_CAP,103) AS FECHA FROM SAM_REQUISIC WHERE SAM_REQUISIC.CVE_REQ = ?",new Object[]{cve_req});
	}
		...
		try {   
			...
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
	            	...
	            	gatewayBitacora.guardarBitacora(gatewayBitacora.CAMBIOS_REQUISICION, ejercicio, cve_pers, Long.parseLong(cve_req.toString()), req.get("NUM_REQ").toString(), "REQ", fecha, req.get("ID_PROYECTO").toString(), req.get("CLV_PARTID").toString(), "Cambió la fecha de la Requisición de "+req.get("FECHA").toString()+" a "+fechaNueva, Double.parseDouble(req.get("TOTAL").toString()));
	            	/*Realiza el cambio*/
	            	getJdbcTemplate().update("UPDATE SAM_REQUISIC SET FECHA_CAP = ? WHERE CVE_REQ = ?", new Object[]{fecha, cve_req});
	            } 
	         });
			return true;
		}
	...
	public boolean OrdenTrabajo(Long cve_req, String cve_benefi, String area,
			String tipo_bien, String marca, String modelo, String placas,
			String num_inv, String color, String usuario, int cve_concurso,
			int idUsuario) {
