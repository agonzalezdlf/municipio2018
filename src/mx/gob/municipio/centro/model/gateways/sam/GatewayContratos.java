...
public class GatewayContratos extends BaseGateway {
	...
	public Long guardarContrato(Long cve_contrato, int idDependencia, String num_contrato, String fecha_ini, String fecha_fin, String oficio, String tiempo_entrega, int tipo, String concepto, Double anticipo, int idRecurso, String clv_benefi, Long cve_doc, int ejercicio, int cve_pers, int idGrupo){
		...
		if(cve_contrato==0){
			...
			//Corregir haciendo el cve_contrato identity, para no asignar claves de contratos
			cve_contrato = this.getJdbcTemplate().queryForLong("SELECT MAX(CVE_CONTRATO) AS N FROM SAM_CONTRATOS")+1;
			
			this.getJdbcTemplate().update("INSERT INTO SAM_CONTRATOS(ID_DEPENDENCIA, ID_GRUPO, ID_RECURSO, CLV_BENEFI, CVE_DOC, NUM_CONTRATO, EJERCICIO, ID_TIPO, CVE_PERS, FECHA, FECHA_INICIO, FECHA_TERMINO, OFICIO_AUT, TIEMPO_ENTREGA, DESCRIPCION, ANTICIPO, STATUS) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", new Object[]{idDependencia, idGrupo, idRecurso, clv_benefi, cve_doc, num_contrato, ejercicio, tipo, cve_pers, fecha, this.formatoFecha(fecha_ini), this.formatoFecha(fecha_fin), oficio, tiempo_entrega, concepto, anticipo, 0});
		...
		try {                 
            this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
                ...
                protected void   doInTransactionWithoutResult(TransactionStatus status) {
                		...
                		Map contrato = getContrato(cve_contrato);//518
                		...
                		if(contrato.get("CVE_DOC")!=null)//Cliclo donde se contrata atravez del presupuesto de otro documento
                		{
                			...
                			if(!requisicion.get("TIPO").toString().equals("7")||!requisicion.get("TIPO").toString().equals("8"))// SI LA REQUISICION ES ANUALIZADA

                			{
                				
	                			for(Map row: conceptos){
	                				
	                				if(contrato.get("ID_TIPO").toString().equals("7"))//ADQUISICIONES
	                				{
	                				...
	                				if(contrato.get("ID_TIPO").toString().equals("13"))//VALE
	                				{
	                				...
	                				if(contrato.get("ID_TIPO").toString().equals("3")||contrato.get("ID_TIPO").toString().equals("5"))//os u ot
	                				{
	                				...
	                				if(importe.doubleValue() > comprometido.doubleValue()){
	                					System.out.println("Por contrato: " +importe);
	                					System.out.println("Por contrato: " +comprometido);
	                					tiene = false;
                			...
                			else//SI LA REQUISICION ES ANUALIZADA
                			{
                		...
                		}//Termina el cliclo donde se contrata atravez del presupuesto de otro documento
                		else//HACE EL COMPROMISO POR CONTRATO
                		{
                		...
                		int mes = fecha_cierre.get(Calendar.MONTH) + 1;
                		System.out.println("Mes de cierre del contrato es: " + mes);
                		try {
                			if(tiene){
                    			...
                    			Integer Mes_cerrado =getJdbcTemplate().queryForInt("SELECT dbo.cnt_siMesAbierto('SAM_CONTRATOS','CONTRATOS_INS',?) AS MES_ABIERTO", new Object[]{mes});
                    			try {
									if (Mes_cerrado==1){
		                				...
		                				if(contrato.get("ID_TIPO").toString().equals("3")){
		                					getJdbcTemplate().update("UPDATE SAM_REQUISIC SET FECHA_FINIQUITADO =?, MES_FINALIZADO = ?, DIA_FINALIZADO = ? WHERE CVE_REQ = ?",new Object[]{fecha, fecha.getMonth()+1, fecha.getDay(), contrato.get("CVE_DOC")});
		                				}
                    			...
                    			//gatewayBitacora.guardarBitacora(gatewayBitacora.CON_CERRO, ejercicio, cve_pers, cve_contrato, num, "CON", fecha, detalle.get("ID_PROYECTO"), clv_partid, null, timporte);
                    		}
						} catch (Exception e) {
							// TODO: handle exception
							throw new RuntimeException("Error para cerrar el contrato");
						}
	...
	public List <Map> getListaContratos(Long cve_contrato, String num_contrato, int idDependencia, int mesActivo, int idRecurso){
		...
		return this.getJdbcTemplate().queryForList("SELECT  B.CVE_CONTRATO, "+
																...
																" GROUP BY B.CVE_CONTRATO, B.NUM_CONTRATO, B.DESCRIPCION, A.PERIODO, B.ID_RECURSO, D.CLV_BENEFI, D.NCOMERCIA " + 
