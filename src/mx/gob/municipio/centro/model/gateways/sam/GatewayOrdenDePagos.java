...
/*REVISAR PARA NO APERTURAR UNA ORDEN DE PAGO QUE ESTE EJERCIDA*/
...
public List<Map> getListadoOrdenesPagoenProgramacion(int mes, int ejercicio){
	Map parametros =  new HashMap<String,Object>();
	/*Asignacion de parametros*/
	parametros.put("ejercicio", ejercicio);
	parametros.put("mes", mes);
	
	String sql = "SELECT OP.CVE_OP,OP.NUM_OP,OP.TIPO,OP.ID_RECURSO,OP.ID_DEPENDENCIA,B.NCOMERCIA,OP.NOTA,OP.IMPORTE,convert(varchar(10),OP.FECHA ,103)FECHA, convert(varchar(10),OP.FECHA_CIERRE,103)FECHA_CIERRE, OP.PERIODO,YEAR(OP.FECHA)ANIO,MONTH(OP.FECHA)MES,DAY(OP.FECHA)DIA,convert(varchar(10),OP.F_REC_PROG,103)F_REC_PROG " +
				 " FROM SAM_ORD_PAGO OP "+ 
				 " INNER JOIN CAT_BENEFI B ON (B.CLV_BENEFI = OP.CLV_BENEFI) "+ 
				 " WHERE OP.STATUS=0 ";
	if(mes!=0) sql += " and MONTH(OP.FECHA)=:mes";
	sql += " ORDER BY OP.CVE_OP ASC";
...
public void ejercerOrdenPagoFinal(Long cve_op, Date fecha_ejerce, int ejercicio, int cve_pers) throws ParseException{
		...
		*/
		...
		//Modificacion del 2018---- ID_POLIZA_CH, ID_PAGO_ELEC, DESC_PART, DESCPART, CHEQUE, FE_PASIVO, COMPROBADA ----- ESOS CAMPOS NO OPERAN YA----------------
		SQL = "INSERT INTO ORDENDPAGO(ID_OP,ID_RECURSO,ID_DEPENDENCIA,CLV_BENEFI,FECHA,FECHA_EJER,REEMBOLSOF,CONCURSO,IMPORTE,RETENCION, IMPNETO,CUOTA_ISSET,NOTA,FEINI_TSOR,FECHA_PAGO,FE_CONPAS,FE_CONTAB,FE_EGRESOS,MES,MES_PAGO,FEINI_FINANZAS,IVA,IMPORTE_IVA,FE_CANCELA_EJER) "+
			  "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		this.getJdbcTemplate().update(SQL, new Object[]{cve_op, OrdenPago.get("ID_RECURSO"), OrdenPago.get("ID_DEPENDENCIA"), OrdenPago.get("CLV_BENEFI"), OrdenPago.get("FECHA"), (Date) fecha_ejerce, OrdenPago.get("REEMBOLSOF"), concurso, OrdenPago.get("IMPORTE"), OrdenPago.get("RETENCION"), impNeto, 0, OrdenPago.get("NOTA"), null, null, null, null, null, sMes.toUpperCase(), null, null, OrdenPago.get("IVA"), OrdenPago.get("IMPORTE_IVA"),null});
...
public boolean cambiarFechaOrdenPago(Long cve_op, String fechaNueva, int ejercicio, int cve_pers){
	...
	//Validacion de Orden de Pago ingresada en la Dirección de Finanzas
	...
	public void ingresoOPProgramacion(Long cve_op, String fecha, int ejercicio, int cve_pers){
		Date fechaActual = new Date();
		String folio=rellenarCeros(cve_op.toString(),6);
		this.getJdbcTemplate().update("UPDATE SAM_ORD_PAGO SET F_REC_PROG=? WHERE CVE_OP=?", new Object[]{this.formatoFecha(fecha), folio});
		//this.formatoFecha
		gatewayBitacora.guardarBitacora(gatewayBitacora.OP_VALIDA_FINANZAS, ejercicio, cve_pers, cve_op, folio, "OP", fechaActual, null, null, "", Double.parseDouble("0".toString()));
	}
	
	//Validacion de Orden de Pago ingresada en la Dirección de Programación
		public void DevolucionOP(Long cve_op, String fecha, int ejercicio, int cve_pers){
			Date fechaActual = new Date();
			String folio=rellenarCeros(cve_op.toString(),6);
			this.getJdbcTemplate().update("UPDATE SAM_ORD_PAGO SET F_REC_PROG=?,F_DEV_PROG=? WHERE CVE_OP=?", new Object[]{this.formatoFecha(fecha),fechaActual, folio});
			//this.formatoFecha
			gatewayBitacora.guardarBitacora(gatewayBitacora.OP_VALIDA_FINANZAS, ejercicio, cve_pers, cve_op, folio, "OP", fechaActual, null, null, "", Double.parseDouble("0".toString()));
		}
