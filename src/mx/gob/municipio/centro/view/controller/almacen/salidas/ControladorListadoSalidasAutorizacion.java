...
public class ControladorListadoSalidasAutorizacion extends ControladorBaseAlmacen{
	...
	public List<Map> getListadoDocumentosAutorizacion(Map parametros){
		String sql = "SELECT ENTRADAS.*, VPROYECTO.N_PROGRAMA, SAM_PEDIDOS_EX.CVE_PED, SAM_PEDIDOS_EX.NUM_PED, CONVERT(varchar(10), ENTRADAS.FECHA,103) AS FECHA_CREACION, "
				+ "((SELECT SUM(M.CANTIDAD) FROM DETALLES_ENTRADAS AS M WHERE M.ID_ENTRADA = ENTRADAS.ID_ENTRADA) - (SELECT ISNULL(SUM(M.CANTIDAD) ,0) FROM DETALLE_SALIDA AS M INNER JOIN SALIDAS AS S ON (S.ID_SALIDA=M.ID_SALIDA) AND M.STATUS=1 AND S.ID_ENTRADA = ENTRADAS.ID_ENTRADA))E,(CASE ENTRADAS.STATUS WHEN 1 THEN 'CERRADO' WHEN 0 THEN 'CANCELADO' END) AS STATUS_DESC, CAT_BENEFI.NCOMERCIA FROM ENTRADAS LEFT JOIN CAT_BENEFI ON (CAT_BENEFI.CLV_BENEFI = ENTRADAS.ID_PROVEEDOR) INNER JOIN VPROYECTO ON (VPROYECTO.ID_PROYECTO = ENTRADAS.ID_PROYECTO) LEFT JOIN SAM_PEDIDOS_EX ON (SAM_PEDIDOS_EX.CVE_PED = ENTRADAS.ID_PEDIDO) ";
		String clausulas = "WHERE ENTRADAS.STATUS IN (1) AND ENTRADAS.FECHA_CIERRE IS NOT NULL AND (SELECT SUM(M.CANTIDAD) FROM DETALLES_ENTRADAS AS M WHERE M.ID_ENTRADA = ENTRADAS.ID_ENTRADA) > (SELECT ISNULL(SUM(M.CANTIDAD) ,0) FROM DETALLE_SALIDA AS M INNER JOIN SALIDAS AS S ON (S.ID_SALIDA=M.ID_SALIDA) AND M.STATUS=1 AND S.ID_ENTRADA = ENTRADAS.ID_ENTRADA) ";
